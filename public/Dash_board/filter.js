window.addEventListener('DOMContentLoaded', function() {
    ...
    var min_price = params.get('h');
    var max_price = params.get('l');
    // console.log(categoryName);

    let prod_catalogue = document.getElementById("prod_catag");

    fetch(`http://localhost:3000/products/?$or[0][productName]=${search_value}&$or[1][productCategory]=${search_value}&productPrice[gte]=${min_price}&productPrice[lte]=${max_price}`)
    .then(res => res.json())
    .then(json => {
...
document.getElementById('priceRanges').addEventListener('click', function(event) {
  ...
  function ascending(){
    ...
    fetch(`http://localhost:3000/products/?$or[0][productName]=${search_value}&$or[1][productCategory]=${search_value}&productPrice[gte]=${min_price}&productPrice[lte]=${max_price}&sort=productPrice`)
    .then(res => res.json())
    .then(json => {
        if (json.status === 'success' && json.length != 0) {
            console.log(json);
